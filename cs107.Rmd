...
In this work we'll use a dataset that we obtained thankfully from Julian McAuley, at the University of San Diego (here)[http://jmcauley.ucsd.edu/data/amazon/] 

We'll be also inspired by their SIGIR and KDD papers (listed on the above page) as a baseline for our accuracy benchmarking.
...
this.dir <- "/mnt/hgfs/final project/cs107final/data/"
setwd(this.dir)

#json_file <- "data/reviews_electronics_extract.json"
json_file <- "reviews_electronics_extract.json.gz"
```
...
#dat <- dat %>% head(2000)    ## comment after testing
...
In the following, we'll experiment with a 1-star rated wireless charger from Amazon. And to evaluate this chuncker we selected a text with a lot of grammer and orthograph mistakes:

Fix punctuation issues in this example "best.I" using `gsub` from the `stringr` package.

```{r}
txt <- c("I would rather not have to worry about overheating and potential battery damage than to continue using it.")
...
chunks <- annotate(s, Maxent_Chunk_Annotator(), a3)
...
```{r}
words <- subset(chunks, type == "word")

chk_df <- as.data.frame(words) %>% 
  mutate(word = s[words],
         pos_tag = sapply(features, '[[', "POS"),
         chk_tag = sapply(features, '[[', "chunk_tag")
         ) %>% 
  dplyr::select (word,pos_tag, chk_tag)
...
Now we want to identify features. For the purpose of this work we won't go for a full analysis using penn tree. Rather we'll simply identify the following words indise a verbal phrase:
...
inv_ <- data.frame(word = as.character(inv_), score = rep(-1, length(inv_)), stringsAsFactors = F)

## incrementer/decrementers
inc_ <- c("too", "very", "extremly", "badly", "poorly" )
inc_ <-  data.frame(word = as.character(inc_), score = c(2,2,3,.5,.4), stringsAsFactors = F)

dico <- rbind(inv_,inc_ , dict)

## let's apply the scores
chk_df <- chk_df %>% left_join(dico, by="word" )
VPs <- chk_df %>% filter(chk_tag %in% c("B-NP", "I-NP", "O")) %>% 
      mutate(pos_tag = substring(pos_tag,1,2),
             chk_tag = substring(chk_tag,1,1))
VPs$score[is.na(VPs$score)] <- 0
...
```{r}
## chunks array of score
chunk_scores <- c()

## initialize the chunk scope variables
chunk_scope <- FALSE
chunk_score <- 1
chunk_mltpl <- 1
...
for (i in 1:nrow(VPs)) {
  ...
  if (!chunk_scope) {
     # continue looking for a B-VP
     if (r$chk_tag != "B") next 
     chunk_scope <- T
  } else if (r$chk_tag == "B" | r$chk_tag == "O") {
    chunk_scores <- c(chunk_scores, chunk_score*chunk_mltpl)
    chunk_scope <- FALSE
    chunk_score <- 1
    chunk_mltpl <- 1
  }
  # we are inside a chunk scope, analyze until we find a B-VP or O
  if (r$pos_tag == "NN") {
           chunk_score <- chunk_score + r$score
  } else if (r$pos_tag == "JJ" & r$score != 0) {
           chunk_mltpl <- chunk_mltpl * r$score
  } 
}
...
estimate_sentiment = function(x) {
      ...
      chunks <- annotate(x, Maxent_Chunk_Annotator(), a3)
      ## Transform it  a word, pos-tag, chunk-tag dataframe
      words <- subset(chunks, type == "word")
      chk_df<-as.data.frame(words) %>% 
        			mutate(	  word = x[words],
               			pos_tag = sapply(features, '[[', "POS"),
      ...
      ## let's apply the scores
      chk_df <- chk_df %>% left_join(dico, by="word" )
      VPs <- chk_df %>% filter(chk_tag %in% c("B-VP", "I-VP", "O")) %>% 
            mutate(pos_tag = substring(pos_tag,1,2),
                   chk_tag = substring(chk_tag,1,1))
      VPs$score[is.na(VPs$score)] <- 0
      print (VPs)
      ...
      for (i in 1:nrow(VPs)) {
        ...
        if (!chunk_scope) {
          ...
          this.score  <- chunk_score*chunk_mltpl
          pos.count <- pos.count + (this.score >= 0)
          neg.count <- pos.count + (this.score <  0)
          print (this.score)
          chunk_scope <- FALSE
          chunk_score <- 1
          chunk_mltpl <- 1
        ...
        if (r$pos_tag == "VB") {
                 chunk_score <- chunk_score + r$score
        } else if (r$pos_tag == "RB" & r$score != 0) {
                 chunk_mltpl <- chunk_mltpl * r$score
        } 
      }
      
      ## our final score is
      (neg.count - pos.count) /(neg.count + pos.count + 1)
}
...
# xtract <- df %>% sample_n(20)
# y_hat <- apply(xtract, 1, function(z) estimate_sentiment(z))
# table(xtract$rating,  (y_hat > 0) + 0)
# accuracy(xtract$rating,  (y_hat > 0.5) + 0)
