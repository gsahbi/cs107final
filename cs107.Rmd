...
SVMs were developed by Cortes & Vapnik (1995) for binary classification. Their
...
```{r}
## svm
svm.model <- svm(mat_train[,-12],           ## these are being the features
                 as.factor(mat_train[,12]),  ## this is the truth y
                 cost = 1, gamma = 1)
svm.pred <- predict(svm.model, mat_train[,-12])

## compute svm confusion matrix
table(as.factor(mat_test[,12]), svm.pred)
